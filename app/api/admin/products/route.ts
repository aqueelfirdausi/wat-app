...
import { createAppwriteProductMutationService } from "@/lib/appwrite/product-mutations";
...
function dependencies() {
  return {
    ...
    service: createAppwriteProductMutationService()
  };
}
...
async function run(request: Request, operation: "create" | "update" | "delete") {
  if (getServerBackendMode() !== "appwrite") {
    return NextResponse.json(
      { ok: false, code: "NOT_FOUND", error: "Product mutation path unavailable." },
      { status: 404 }
    );
  }
  if (!isMutationEnabled()) return mutationDisabledResponse();
  const result = await handleProductMutationRequest(request, operation, dependencies());
